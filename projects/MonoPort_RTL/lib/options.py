...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        g_data.add_argument('--root', type=str, default='../../data/',
                            help='path to images (data folder)')
        g_data.add_argument('--dataroot_buff', type=str, default=None)

        g_data.add_argument('--loadSize', type=int, default=512, help='load size of input image')
        g_data.add_argument('--inputSize', type=int, default=512, help='input size of input image')
        ...
        g_train.add_argument('--optim', type=str, default="RMSprop", choices=["RMSprop", "Adadelta", "SGD", "Adam"], help='')
        g_train.add_argument('--learning_rate', type=float, default=1e-3, help='adam learning rate')
        g_train.add_argument('--weight_decay', type=float, default=0, help='')
        g_train.add_argument('--momentum', type=float, default=0, help='')
        ...
        g_train.add_argument('--freq_recon', type=int, default=100, help='freqency of the recon mesh')
       
        g_train.add_argument('--no_gen_mesh', action='store_true')
        g_train.add_argument('--no_num_eval', action='store_true')
        g_train.add_argument('--no_metric_eval', action='store_true')
        g_train.add_argument('--no_train', action='store_true')
        g_train.add_argument('--preprocess_sample', action='store_true')
        ...
        g_sample.add_argument('--sigma_geo', type=float, default=0.05, help='perturbation standard deviation for positions')
        g_sample.add_argument('--sigma_color', type=float, default=0.001, help='perturbation standard deviation for positions')

        g_sample.add_argument('--num_sample_geo', type=int, default=5000, help='# of sampling points')
        ...
        g_model.add_argument('--gtype', type=str, default='HGPIFuNet') # HGPIFuNet | ConvPIFuNet

        # hrnet filter specify
        g_model.add_argument('--netIMF', type=str, default='HRNetV2_W18_small_v2_balance')
        ...
        # CBN classifier
        g_model.add_argument('--classifierIMF', type=str, default="SurfaceClassifier")
        g_model.add_argument('--filter_dim', type=int, default=256)
        g_model.add_argument('--cly_dim', type=int, default=3)
        ...
        # soft one hot z encode
        parser.add_argument('--soft_onehot', action='store_true')
        parser.add_argument('--soft_dim', type=int, default=64)
